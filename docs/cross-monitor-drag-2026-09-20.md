...
0.1.12 继续修复了多显示器下的快捷键作用域。Win+1、Win+2、Win+3 等数字槽位
现在读取 `Hyprland.focusedMonitor.name`，并只从当前焦点显示器的工作区模型中解析。
因此在 2 号显示器获得焦点时，Win+1 不会再跳到 1 号显示器的第一个工作区。

MRU 排序、系统原生排序和每个显示器自己的 `New workspace` 卡片都使用同一规则。
